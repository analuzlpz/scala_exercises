...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw new Exception("There is no tweets in a Empty Set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet = {
    lazy val left_that = that.filter(x => x.text < elem.text)
    lazy val right_that = that.filter(x => x.text >= elem.text)
    lazy val new_left = left.union(left_that)
    lazy val new_right = right.union(right_that)
    new NonEmpty(elem,new_left,new_right)
    }
  ...
  def mostRetweeted: Tweet =
    lazy val left_mostRetweeted = left.mostRetweeted
    lazy val right_mostRetweeted = right.mostRetweeted

    if (!left.isEmpty() && elem.retweets < left_mostRetweeted.retweets)
      if (!right.isEmpty() && elem.retweets < right_mostRetweeted.retweets)
        right_mostRetweeted
      else
        left_mostRetweeted
    else if (!right.isEmpty() && right_mostRetweeted.retweets > elem.retweets )
      right_mostRetweeted
    else
      elem
